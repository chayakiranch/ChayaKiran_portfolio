  ...
  return (
    <header className="sticky top-0 z-50 px-4 pt-4">
      <div className="max-w-[1120px] mx-auto flex items-center justify-between gap-4 bg-panel/80 backdrop-blur-md border border-panel-border rounded-full px-3 py-2 shadow-lg">
        ...
        <button
          className="md:hidden w-8 h-8 flex items-center justify-center text-text mr-1"
      ...
      {open && (
        <nav className="md:hidden flex flex-col gap-2 mt-2 mx-auto max-w-[1120px] bg-panel/95 backdrop-blur-md border border-panel-border rounded-2xl p-3">
          {links.map((l) => (
            <NavLink
              key={l.to}
              to={l.to}
